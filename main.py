...
async def serve(
    ...
    registered_signal_handlers: list[signal.Signals] = []
    for sig in watched_signals:
        try:
            loop.add_signal_handler(sig, stop_event.set)
            registered_signal_handlers.append(sig)
        except NotImplementedError:
            signal.signal(
                sig,
                lambda _signum, _frame, *, _loop=loop, _event=stop_event: _loop.call_soon_threadsafe(
                    _event.set
                ),
            )
