...
def _parse_tcp_endpoint(value: str) -> tuple[str, int] | None:
    text = value.strip()
...
async def _open_probe_connection(paths: ProbePaths) -> tuple[asyncio.StreamReader, asyncio.StreamWriter]:
    endpoint = _parse_tcp_endpoint(paths.cli_endpoint)
...
async def _wait_for_probe_ready(paths: ProbePaths, proc: subprocess.Popen[bytes], timeout_s: int) -> None:
    ...
    raise SystemExit(
        f"等待 debug CLI 超时: socket={paths.socket} endpoint={paths.cli_endpoint}"
    )
...
async def _run_probe(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if args.start_agent:
            ...
            await _wait_for_probe_ready(paths, proc, args.start_timeout)
        ...
        session_key = ""
        reader, writer = await _open_probe_connection(paths)
        try:
            for index, turn in enumerate(scenario.turns, 1):
                if turn.get("action") == "consolidate":
                    session_key = await _probe_session_key(paths.sessions_db, session_key)
