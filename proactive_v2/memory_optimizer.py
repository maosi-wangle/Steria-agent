...
- 当前自称名称固定为 `{self_name}`，请保持标题和正文中的自称一致，不要改成其他名字
- 只能包含标题和 bullet 列表
- 不要代码块，不要解释，不要额外说明
...
_SELF_TITLE_RE = re.compile(r"^#\s*(?P<name>.+?)\s*的自我认知", re.MULTILINE)
_SELF_BODY_NAME_RE = re.compile(
    r"(^\s*-\s*我是)\s+(?P<name>[^\s，。,.;:：]+)",
    re.MULTILINE,
)


def _extract_self_name(self_content: str) -> str:
    text = str(self_content or "").strip()
    if not text:
        return "Akashic"

    body_match = _SELF_BODY_NAME_RE.search(text)
    if body_match:
        return str(body_match.group("name") or "").strip() or "Akashic"

    title_match = _SELF_TITLE_RE.search(text)
    if title_match:
        return str(title_match.group("name") or "").strip() or "Akashic"

    return "Akashic"


def _enforce_self_name(self_content: str, self_name: str) -> str:
    text = str(self_content or "").strip()
    name = str(self_name or "").strip() or "Akashic"
    if not text:
        return DEFAULT_SELF_MD.replace("Akashic", name)

    if _SELF_TITLE_RE.search(text):
        text = _SELF_TITLE_RE.sub(f"# {name} 的自我认知", text, count=1)
    else:
        text = f"# {name} 的自我认知\n\n{text}"

    if _SELF_BODY_NAME_RE.search(text):
        text = _SELF_BODY_NAME_RE.sub(rf"\1 {name}", text, count=1)
...
class MemoryOptimizer:
    ...
    async def _update_self(self, pending: str) -> None:
        ...
        if not self_content:
            ...
            return
        self_name = _extract_self_name(self_content)
        ...
        try:
            ...
            if updated:
                self._memory.write_self(_enforce_self_name(updated, self_name))
