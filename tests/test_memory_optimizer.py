...
def test_optimize_preserves_existing_self_name(tmp_path):
    memory = MarkdownMemoryStore(tmp_path)
    memory.write_long_term("old")
    memory.write_self(
        "# Steria 的自我认知\n\n"
    ...
    memory.append_pending("- [preference] 用户喜欢过程信息和最终答案分开。")
    ...
    updated = memory.read_self()
    assert updated.startswith("# Steria 的自我认知")
    assert "- 我是 Steria" in updated
    assert "新版理解" in updated

    self_prompt = provider.chat.await_args_list[1].kwargs["messages"][1]["content"]
    assert "当前自称名称固定为 `Steria`" in self_prompt
