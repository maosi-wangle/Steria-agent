...
async def test_qq_private_trace_sends_forward_then_final_and_clears_state(
    ...
    await event_bus.observe(
        TurnStarted(
            ...
            channel="qq",
            chat_id="1",
            content="show me the latest changes",
    ...
    await event_bus.observe(
        ToolCallStarted(
            ...
            arguments={"description": "check recent messages", "query": "latest changes"},
        )
    )
    await event_bus.observe(
        ToolCallCompleted(
            ...
            tool_name="fetch_messages",
            arguments={"description": "check recent messages", "query": "latest changes"},
            final_arguments={"description": "check recent messages", "query": "latest changes"},
            status="ok",
    ...
    await channel._on_response(
        OutboundMessage(
            channel="qq",
            chat_id="1",
            content="I checked it. The recent changes are mainly about QQ tracing.",
            thinking="First confirm whether this round used tools, then summarize the result.",
            metadata={
                "context_retry": {
                    "llm_context_frame": """<system-reminder data-system-context-frame="true">

## retrieved_memory
- [preference] The user wants process details and the final answer separated.
- [event] The previous round confirmed QQ private merged forward works.
    ...
    assert forward_payload["news"][0]["text"] == "Steria：【模型思路】"
    assert forward_payload["news"][1]["text"] == "Steria：【Recall】"
    assert forward_payload["news"][2]["text"] == "Steria：【工具链】"
    payload_text = str(forward_payload)
    assert "【Recall】" in payload_text
    assert "process details and the final answer separated" in payload_text
    assert "fetch_messages" in payload_text
    assert "fetch_messages" in payload_text
    assert calls[1] == ("text", 1, "I checked it. The recent changes are mainly about QQ tracing.")
